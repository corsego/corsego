...
class Course < ApplicationRecord
  ...
  def update_rating
    if enrollments.where.not(rating: nil).where.not(rating: 0).exists?
      update_column :average_rating, enrollments.where.not(rating: nil).where.not(rating: 0).average(:rating).round(2).to_f
    else
